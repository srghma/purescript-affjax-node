...
main = void $ runAff (either (\e -> logShow e *> throwException e) (const $ log "affjax: All good!")) do
  ...
  { server, port } <- fromEffectFnAff startServer
    ...
    (AN.request $ AX.defaultRequest { url = mirror }) >>= assertRight >>= \res -> do
      assertEq ok200 res.status

    A.log "GET /does-not-exist: should be 404 Not found"
    (AN.request $ AX.defaultRequest { url = doesNotExist }) >>= assertRight >>= \res -> do
      assertEq notFound404 res.status

    A.log "GET /not-json: invalid JSON with Foreign response should return an error"
    AN.get ResponseFormat.json doesNotExist >>= assertLeft >>= case _ of
    ...
    AN.get ResponseFormat.string notJson >>= assertRight >>= \res -> do
      assertEq ok200 res.status

    A.log "GET /slow with timeout: should return an error"
    (AN.request $ AX.defaultRequest { url = slow, timeout = Just (Milliseconds 100.0) }) >>= assertLeft >>= case _ of
    ...
    AN.post ResponseFormat.json mirror (Just (RequestBody.string "test")) >>= assertRight >>= \res -> do
      assertEq ok200 res.status
      assertEq (Just "POST") (J.toString =<< FO.lookup "method" =<< J.toObject res.body)

    A.log "PUT with a request body"
    let content = "the quick brown fox jumps over the lazy dog"
    AN.put ResponseFormat.json mirror (Just (RequestBody.string content)) >>= assertRight >>= \res -> do
      assertEq ok200 res.status
